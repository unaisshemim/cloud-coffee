...
export function detectJsonImportType(parsed: unknown): ImportType {
	...
	// JSON Resume standard: top-level `basics`, without cloudcoffee's `sections`/`metadata`.
	if ("basics" in data && !("sections" in data) && !("metadata" in data)) return "json-resume-json";

	// cloudcoffee exports carry `sections` + `metadata`; the current schema's metadata has a `page` key,
