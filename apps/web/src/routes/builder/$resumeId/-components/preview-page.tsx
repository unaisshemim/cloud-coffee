...
import { DEFAULT_PDF_PAGE_SIZE, getResumePreviewPageCount } from "@/features/resume/preview/preview.shared.utils";
...
export function PreviewPage() {
	...
	useEffect(() => {
		...
		const updateScale = ({ width }: DOMRectReadOnly) => {
			setPageScale(getBuilderPreviewPageScale(width, DEFAULT_PDF_PAGE_SIZE.width));
		};
	...
	return (
		<Suspense fallback={<LoadingScreen />}>
			<div ref={viewportRef} className="absolute inset-0 overflow-hidden bg-[#f7f3ef]">
				<div className="absolute top-5 right-5 z-20">
					<PageNavigator currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} />
				</div>

				<div className="flex h-full items-start justify-center overflow-auto px-6 pt-24 pb-8">
					<ResumePreview
						pageLayout="vertical"
						pageScale={pageScale}
