...
description: "Use cloudcoffee Semantic CSS to make safe, targeted, and portable changes to your resume PDF."
---
...
If a resume still uses the previous Custom Styles form, cloudcoffee creates a converted stylesheet draft. Your
...
<Steps>
  ...
  <Step title="Activate Semantic CSS">
    Select **Activate Semantic CSS** only after the preview matches the legacy result. cloudcoffee never applies both
...
The first line tells cloudcoffee which language version the stylesheet uses. Keep `@version 1;` at the start of
every stylesheet.
...
<Steps>
  ...
  <Step title="Wait for Valid">
    cloudcoffee checks the source in the browser. When the status changes to **Valid**, compare the preview and
...
| `template` | `resume` | `resume[template="treecko"]` |
...
| --- | --- |
| Treecko | `template-part[name="item-header-row"]` |
