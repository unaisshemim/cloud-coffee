...
description: "Explore the cloudcoffee monorepo layout, runtime boundaries between the web and server apps, and the package ownership model."
---

cloudcoffee is a pnpm/Turborepo monorepo. The product runs as one deployed Node.js process, while the source is split into a full-stack web app, a server adapter, and focused internal packages.
...
`apps/web` owns the TanStack Start experience and browser WebMCP tools. `apps/server` owns the production Hono process and mounts RPC, auth, OpenAPI, static uploads, schema JSON, and the built web app.
...
| Browser WebMCP tool behavior | `apps/web/src/features/webmcp` |
...
Browser WebMCP implementation lives in `apps/web/src/features/webmcp`. Page routes register tools through `document.modelContext` while mounted and call protected oRPC procedures for server behavior. The server has no remote MCP transport.
