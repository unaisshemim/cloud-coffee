...
description: "Import a resume from JSON, JSON Resume, PDF, or Microsoft Word files with automatic format detection from the cloudcoffee dashboard."
---

cloudcoffee can create a new resume from several existing file formats. Use import when you are moving from another tool, restoring a backup, or converting an older cloudcoffee file.

## Supported import formats

| Format | Requires AI integration? | Notes |
| --- | --- | --- |
| **cloudcoffee (JSON)** | No | Best option for backups exported from the current version of cloudcoffee. |
...
| **PDF** | Yes | cloudcoffee asks your configured AI provider to parse the file into structured resume data. |
| **Microsoft Word** | Yes | Supports Word documents. cloudcoffee asks your configured AI provider to parse the document. |
...
<Steps>
  ...
  <Step title="Choose your file">
    Select the resume file from your computer. cloudcoffee detects the format automatically from the file's contents
  ...
  <Step title="Import">
    Click **Import**. cloudcoffee creates a new resume and opens it in the builder when the import succeeds.
...
If you have a file exported from cloudcoffee, choose **cloudcoffee (JSON)**. This preserves the most information because the file already matches cloudcoffee's data model.
...
Make sure you selected the correct JSON import type. A cloudcoffee JSON export, a Reactive Resume v4 JSON export, and a JSON Resume file are different formats.
