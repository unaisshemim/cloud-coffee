...
Use this guide when you want an AI client to manage the **Application Tracker** for you. After the cloudcoffee MCP server is connected, the agent can use tools for the same application workflows available in the app: listing applications, creating records, importing rows, updating stages, adding notes, managing follow-ups, attaching documents, and running Application Copilot.
...
<Steps>
  ...
  <Step title="Confirm the application tools are visible">
    Ask your client to list the cloudcoffee tools. You should see tools such as `list_applications`, `create_application`, `update_application`, `attach_application_document`, and `draft_application_message`.
  </Step>
...
<Info>
  Application Copilot tools require the same AI provider setup used by the app. Match scoring and resume tailoring work best when the application has a linked cloudcoffee and a job description.
</Info>
...
```text
Use the cloudcoffee application auto-fill tool on this job posting URL, then create a saved application from the extracted company, role, location, salary, and job description. Tag it with remote and backend.
```
...
```text
Import these application rows into cloudcoffee. Normalize the stages to saved, applied, screening, interview, offer, or rejected. Skip rows that do not have both a company and role, and tell me what was skipped.
...
| Issue | What to do |
| --- | --- |
| The agent cannot see application tools | Reconnect the MCP server and ask the client to refresh tool discovery. Confirm you are connected to cloudcoffee v5.2.2 or later. |
...
| Match scoring or tailoring fails | Link a cloudcoffee to the application and add a job description. Confirm your AI provider is configured. |
