...
cloudcoffee uses AI providers for features such as the optional AI review in the ATS checker and PDF or Word imports.
...
| **API Key** | The key cloudcoffee should use when sending AI requests to that provider. |
...
Click **Save Provider** when the form is complete. cloudcoffee tests the connection automatically as part of the save, so you'll see a success or failure status immediately without a separate click.
...
<Steps>
  <Step title="Click Test">
    cloudcoffee sends a small request to verify the provider, model, base URL, and key.
  </Step>
  ...
  <Step title="Enable the provider">
    Turn on **Use** for the tested provider you want cloudcoffee to use.
  </Step>
</Steps>
...
For AI-assisted imports, see [Importing resumes](/guides/importing-resumes).
