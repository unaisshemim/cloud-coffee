...
description: "Partially update a cloudcoffee with JSON Patch (RFC 6902) operations to add, remove, replace, and move fields without sending the full document."
---
...
All requests require an OAuth access token in the `Authorization: Bearer` header. See [Using the API](/guides/using-the-api) for authentication details.
...
curl -X PATCH "https://rxresu.me/api/openapi/resume/YOUR_RESUME_ID" \
  ...
  -d '{
    "operations": [
      { "op": "replace", "path": "/metadata/template", "value": "treecko" },
