...
description: "A comprehensive guide to self-host cloudcoffee with Docker (Postgres only), including a detailed environment variable reference and troubleshooting tips."
---
...
cloudcoffee can be self-hosted using Docker in a matter of minutes, and this guide will walk you through the process. Here are some of the services you'll need to get started:
...
SMTP_USER=""
SMTP_PASS=""
SMTP_FROM="cloudcoffee <noreply@rxresu.me>"
SMTP_SECURE="false"
...
# Set to "true" for private AWS S3 buckets using BucketOwnerEnforced ownership.
S3_DISABLE_ACL="false"

# --- AI features (optional) ---
# ENCRYPTION_SECRET is required for saved AI providers.
# The rest of cloudcoffee can run without it.
# Generated using `openssl rand -hex 32`
ENCRYPTION_SECRET=""
...
```
  ...
  <Step title="Create compose.yml">
    This setup runs Postgres and cloudcoffee on a private Docker network.
...
```
    ...
    <Note>
      In Docker, the cloudcoffee server listens on <code>PORT</code> and serves both the API and the built web app.
...
```
    ...
    cloudcoffee should now be available at your `APP_URL` (for the example above: `http://localhost:3000`).
...
<CardGroup cols={2}>
  ...
  <Card title="Optional">
    <ul>
      ...
      <li>
        Saved AI providers (<code>ENCRYPTION_SECRET</code>)
      </li>
      <li>
...
```
  ...
  <Accordion title="Email (SMTP, optional)">
    ...
    - **`SMTP_FROM`**: Default from address (for example, `cloudcoffee <noreply@rxresu.me>`).
...
<Accordion title="Storage (S3 or local)">
  ...
  - **`S3_DISABLE_ACL`** defaults to `"false"`. Set it to `"true"` for private AWS S3 buckets using `BucketOwnerEnforced`; access is then controlled by IAM and bucket policy instead of object ACLs.
</Accordion>

<Accordion title="AI features (optional)">
  Saved AI provider management is usable only when **`ENCRYPTION_SECRET`** is configured. The rest of cloudcoffee can run without it.
...
To update your cloudcoffee installation to the latest available version, follow these steps:
...
Regular backups are essential to protect your data. cloudcoffee stores data in two places: the PostgreSQL database and file uploads (either local storage or S3).
...
cloudcoffee exposes a health check endpoint at `/api/health` that verifies the application and its dependencies. It checks **database** and **storage**; if either is unhealthy, the endpoint returns HTTP `503`.
...
This is particularly useful in high-availability setups where you have multiple instances of cloudcoffee. If one instance becomes unhealthy (for example, it loses database or storage connectivity), the reverse proxy will stop routing traffic to it until it recovers.
...
You can manually verify the health of your cloudcoffee instance:
