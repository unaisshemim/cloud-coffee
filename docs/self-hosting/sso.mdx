...
description: "Configure Single Sign-On for self-hosted cloudcoffee using custom OAuth providers like Authentik, Authelia, Keycloak, or any OIDC-compliant IdP."
---

## Overview

cloudcoffee supports custom OAuth providers, allowing you to integrate with enterprise identity providers and self-hosted authentication solutions. This is particularly useful for organizations that want to:
...
cloudcoffee automatically maps user profile data from the OAuth provider. The following fields are used:

| cloudcoffee Field | OAuth Profile Fields (in order of preference) |
| --------------------- | --------------------------------------------- |
...
```
    ...
    <Warning>
      Use the **plain text** secret in cloudcoffee's environment, not the hashed version used in Authelia's configuration.
    </Warning>
...
<CardGroup cols={2}>
  <Card title="Use HTTPS" icon="lock">
    Always use HTTPS for both your cloudcoffee instance and OAuth provider in production. OAuth tokens should never
  ...
  <Card title="Review scopes" icon="list-check">
    Only request the scopes you need. The default (`openid profile email`) is sufficient for cloudcoffee.
  </Card>
</CardGroup>
