...
description: "Run cloudcoffee as a self-hosted resume builder with Docker Compose, PostgreSQL, optional object storage, Single Sign-On, and v4 to v5 migration."
---

cloudcoffee can be run as a self-hosted resume builder when you want to operate the app on your own infrastructure instead of using the hosted instance.
...
- cloudcoffee application container.
- PostgreSQL database.
...
Some features require extra configuration. For example, saved AI providers need server-side encryption configuration. Self-hosted deployments can also expose API endpoints from their own domain.

For automation setup on self-hosted instances, see [Using the API](/guides/using-the-api).

<Tip>
  Replace hosted URLs such as <code>https://rxresu.me</code> with your own instance URL when following API or
  sharing examples.
</Tip>
